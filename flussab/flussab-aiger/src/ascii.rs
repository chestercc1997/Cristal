    ...
    L: Lit,
{
    ...
    pub fn print_symbols(&mut self, symbol: &Symbol) {
        let (prefix, index) = match symbol.target {
            SymbolTarget::Input(index) => (b"i", index),
            SymbolTarget::Output(index) => (b"o", index),
            SymbolTarget::Latch(index) => (b"l", index),
        ...
        println!("{}{}: {}", String::from_utf8_lossy(prefix), index, symbol.name);
    }
    pub fn print_lit(&self, lit: L) {
        // 打印 lit 的代码
        println!("{}", lit.code()); // 假设 lit.code() 返回一个实现了 Display trait 的值
    }
    pub fn collect_symbols(&mut self, symbol: &Symbol) -> (Vec<String>, Vec<String>) {
        let mut input_names = Vec::new();
        let mut output_names = Vec::new();
    
        match &symbol.target {
            SymbolTarget::Input(_) => {
                input_names.push(symbol.name.to_string()); // Convert Cow to String
            }
            SymbolTarget::Output(_) => {
                output_names.push(symbol.name.to_string()); // Convert Cow to String
            }
            // You can handle other targets if needed
            _ => {}
        }
    ...
    pub fn print_header(&mut self, header: &Header) {
        let fields = [
            header.max_var_index,
            header.input_count,
            header.latch_count,
    ...
    pub fn print_symbol(&mut self, aig: &OrderedAig<L>) {
        for symbol in &aig.symbols {
            self.print_symbols(symbol);
        }
    }
    pub fn collect_symbol(&mut self, aig: &OrderedAig<L>) -> (Vec<String>, Vec<String>) {
        let mut input_names = Vec::new();
        let mut output_names = Vec::new();
    
        for symbol in &aig.symbols {
            let (inputs, outputs) = self.collect_symbols(symbol);
            input_names.extend(inputs);   // Add input names from the collected symbols
            output_names.extend(outputs);  // Add output names from the collected symbols
        }
    
        (input_names, output_names) // Return both vectors
    }
    pub fn print_aig_header(&mut self, aig: &OrderedAig<L>) {
        // 调用 print_header 来打印 header
