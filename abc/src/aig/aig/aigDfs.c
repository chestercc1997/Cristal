...
int Aig_ManSuppSize_rec( Aig_Man_t * p, Aig_Obj_t * pObj )
{
    // 已访问节点直接返回
    if ( Aig_ObjIsTravIdCurrent(p, pObj) )
        return 0;
    ...
    // 标准AND节点递归两个输入
    assert( Aig_ObjIsNode(pObj) );
    return Aig_ManSuppSize_rec(p, Aig_ObjFanin0(pObj)) + 
           Aig_ManSuppSize_rec(p, Aig_ObjFanin1(pObj));
}


int Aig_ManSuppSize(Aig_Man_t* p, int ** pNodes, int nNodes) {
    int i, Counter = 0;
    Aig_Obj_t* pObj;
    
    // Setup traversal
    Aig_ManIncrementTravId(p);
    Aig_ObjSetTravIdCurrent(p, Aig_ManConst1(p));  // Mark constant
    
    for (i = 0; i < nNodes; i++) {
        pObj = Aig_ManObj( p, pNodes[i] );
        if (Aig_ObjIsCo(pObj))  // CO: traverse its driver
            Counter += Aig_ManSuppSize_rec(p, Aig_ObjFanin0(pObj));
        else  // Regular node
            Counter += Aig_ManSuppSize_rec(p, pObj);
    }
    return Counter;
}
