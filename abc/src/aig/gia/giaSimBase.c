...
printf( "[DATA] matches=%d(%.2f%%) | mismatches=%d(%.2f%%) | boundary=%d(%.2f%%)\n", 
...
static float PearsonCorrelation(int *depth_subj, int *depth_cand, int m) {
    double sumSubj = 0, sumCand = 0;
    double sumSubjCand = 0, sumSubj2 = 0, sumCand2 = 0;
    
    // 累计所有节点的数值
    for (int i = 0; i < m; i++) {
        sumSubj    += depth_subj[i];
        sumCand    += depth_cand[i];
        sumSubjCand += depth_subj[i] * depth_cand[i];
        sumSubj2   += depth_subj[i] * depth_subj[i];
        sumCand2   += depth_cand[i] * depth_cand[i];
    }
    
    // 根据公式计算皮尔逊相关系数 r
    double numerator   = sumSubjCand - (sumSubj * sumCand) / m;
    double denominator = sqrt((sumSubj2 - (sumSubj * sumSubj) / m) * 
                              (sumCand2 - (sumCand * sumCand) / m));
    
    float r = (denominator == 0) ? 0 : (float)(numerator / denominator);
    
    // 结构不相似度为 1 - |r|
    return 1.0f - fabs(r);
}

void Gia_ManCompareSimsEx( Gia_Man_t * pHie, Gia_Man_t * pFlat, int nWords, int fVerbose )
{
    abctime clk = Abc_Clock();
    
    // ========== 新增关键初始化 ==========
    Gia_ManStaticFanoutStart(pFlat);  // 扇出信息初始化
    Gia_ManStaticFanoutStart(pHie);
    Gia_ManLevelNum(pFlat);           // 逻辑深度初始化
    Gia_ManLevelNum(pHie);
    ...
    Vec_Int_t * vLabels = Vec_IntStartFull( Gia_ManObjNum(pFlat) );
    Gia_Obj_t * pObj; int fCompl, Value, * pSpot, * pSpot2, i, nC0s = 0, nC1s = 0, nUnique = 0, nFound[3] = {0}, nBoundary = 0, nMatched = 0;
    Vec_Mem_t * vStore = Vec_MemAlloc( nWords, 12 );
    ...
    // ========== 新增结构指标计算 ==========
    // 获取节点数
    int nFlat = Gia_ManCandNum(pFlat);
    int nHie = Gia_ManCandNum(pHie);
    int nMin = Abc_MinInt(nFlat, nHie);

    // 特征收集
    NodeFeature_t *ftFlat = ABC_CALLOC(NodeFeature_t, nFlat);
    NodeFeature_t *ftHie  = ABC_CALLOC(NodeFeature_t, nHie);
    
    Gia_ManForEachCand(pFlat, pObj, i) {
        if (i >= nFlat) break; // 防御性边界检查
        ftFlat[i].iDepth  = Gia_ObjLevel(pFlat, pObj);
        ftFlat[i].nFanout = Gia_ObjFanoutNum(pFlat, pObj);
    }
    Gia_ManForEachCand(pHie, pObj, i) {
        if (i >= nHie) break; // 防御性边界检查
        ftHie[i].iDepth  = Gia_ObjLevel(pHie, pObj);
        ftHie[i].nFanout = Gia_ObjFanoutNum(pHie, pObj);
    }

    // 计算相关系数
    int *depthFlat = ABC_CALLOC(int, nMin);
    int *depthHie  = ABC_CALLOC(int, nMin);
    for(int i = 0; i < nMin; i++) {
        depthFlat[i] = (i < nFlat) ? ftFlat[i].iDepth : 0;
        depthHie[i]  = (i < nHie)  ? ftHie[i].iDepth  : 0;
    }
    float depth_corr = PearsonCorrelation(depthFlat, depthHie, nMin);

    // AND门差异计算
    int and_diff = abs(Gia_ManAndNum(pFlat) - Gia_ManAndNum(pHie));
    float and_ratio = (float)and_diff / (float)Abc_MaxInt(1, Abc_MaxInt(Gia_ManAndNum(pFlat), Gia_ManAndNum(pHie)));
    ...
    printf( "[STRUCTURE] DepthCorr=%.2f ANDdiff=%d(%.2f%%)\n", 
        depth_corr, and_diff, and_ratio*100 );
    ...
    ABC_FREE(depthFlat);
    ABC_FREE(depthHie);
    Gia_ManStaticFanoutStop(pFlat);  // 必须与Start成对出现
    Gia_ManStaticFanoutStop(pHie);
