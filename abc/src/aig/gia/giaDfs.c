...
/**Function*************************************************************
  ...
  Synopsis    [Collects ANDs and CIs nodes.]

  Description [This function collects both AND nodes and CI nodes for a given set of nodes.
               AND nodes are stored in vAnds, while CI nodes are stored in vCis.]
...
***********************************************************************/
void Gia_ManCollectAndsCis( Gia_Man_t * p, int * pNodes, int nNodes, Vec_Int_t * vAnds, Vec_Int_t * vCis )
{
    int i, iLeaf; 
    Gia_Obj_t * pObj;

    // Initialize traversal ID
    Gia_ManIncrementTravId( p );
    Gia_ObjSetTravIdCurrentId( p, 0 );

    // Clear the vectors
    if (vAnds) Vec_IntClear(vAnds);
    if (vCis) Vec_IntClear(vCis);

    // Traverse the given nodes
    for (i = 0; i < nNodes; i++) {
        pObj = Gia_ManObj( p, pNodes[i] );

        // If it's a CO, traverse its fanin
        if (Gia_ObjIsCo(pObj)) {
            Gia_ManCollectAndsCis_rec(p, Gia_ObjFanin0(pObj), vAnds, vCis);
        }
        // If it's an AND node, traverse it
        else if (Gia_ObjIsAnd(pObj)) {
            Gia_ManCollectAndsCis_rec(p, pNodes[i], vAnds, vCis);
        }
        // Otherwise, collect it as a CI
        else if (Gia_ObjIsCi(pObj)) {
            if (vCis) Vec_IntPush(vCis, Gia_ObjId(p, pObj));
        }
    }
}
...
/**Function*************************************************************
  ...
  Synopsis    [Recursive helper function to collect AND and CI nodes.]

  Description [This recursive function is used to traverse the structure
               and collect AND nodes into vAnds and CI nodes into vCis.]
...
***********************************************************************/
void Gia_ManCollectAndsCis_rec( Gia_Man_t * p, int iObj, Vec_Int_t * vAnds, Vec_Int_t * vCis )
{
    Gia_Obj_t * pObj;
    ...
    // If it's a CI node, add it to vCis
    if (Gia_ObjIsCi(pObj)) {
        if (vCis) Vec_IntPush(vCis, Gia_ObjId(p, pObj));
        return;
    }

    // If it's an AND node, traverse its fanins and add it to vAnds
    if (Gia_ObjIsAnd(pObj)) {
        Gia_ManCollectAndsCis_rec(p, Gia_ObjFaninId0(pObj, iObj), vAnds, vCis);
        Gia_ManCollectAndsCis_rec(p, Gia_ObjFaninId1(pObj, iObj), vAnds, vCis);
        if (vAnds) Vec_IntPush(vAnds, iObj);
    }
}
