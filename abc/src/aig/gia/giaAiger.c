...
//            pCur++;
        while ( pCur < (unsigned char *)pContents + nFileSize )
        {
            ...
            else if ( *pCur == 'e' )
            {
                ...
                // if (1){
                //     Gia_PrintNextsWithSupport(pNew,pNew->pNexts, Gia_ManObjNum(pNew));
                // }
            }
            // read flop classes
            else if ( *pCur == 'f' )
...
Gia_Rpr_t *CreateReprsWithInputs(Gia_Man_t *pNew, int *rootNodes, int numRoots, int *inputNodes, int numInputs, int *danglingLenNodes, int numDanglingLen, int nSize)
{
    // 检查 danglingLenNodes 和 numRoots 是否匹配
    if (numDanglingLen != numRoots)
    {
        printf("Error: numDanglingLen (%d) does not match numRoots (%d).\n", numDanglingLen, numRoots);
        return NULL;
    }

    // 分配 pReprs 数组并初始化
    Gia_Rpr_t *pReprs = ABC_CALLOC(Gia_Rpr_t, nSize);
    for (int i = 0; i < nSize; i++)
    {
        pReprs[i].iRepr = GIA_VOID; // 初始化为 GIA_VOID
    }
    // printf("Root nodes: ");
    // for (int i = 0; i < numRoots; i++) {
    //     printf("%d ", rootNodes[i]);
    // }
    // printf("\n");
    // 遍历 root nodes 和 input nodes，按动态长度处理
    int inputIndex = 0; // 当前 inputNodes 的索引
    for (int i = 0; i < numRoots; i++) // 遍历每个 root 节点
    {
        int rootNode = rootNodes[i];             // 当前 root 节点
        int groupSize = danglingLenNodes[i];     // 当前 root 节点的 inputNodes 数量

        //printf("Processing rootNode %d with groupSize %d\n", rootNode, groupSize);

        // 处理对应的 input 节点组
        for (int j = 0; j < groupSize; j++)
        {
            if (inputIndex > numInputs) // 防止越界
            {
                // 分别打印错误信息
                printf("Error: inputIndex exceeds numInputs.\n");
                printf("  inputIndex: %d\n", inputIndex);
                printf("  numInputs: %d\n", numInputs);
                return NULL;
            }

            int inputNode = inputNodes[inputIndex++];
            pReprs[inputNode].iRepr = rootNode; // 当前 input 节点的代表是 rootNode
            pReprs[inputNode].fProved = 0;     // fProved 初始化为 0
        }

        // rootNode 不属于任何其他节点的等价类
        pReprs[rootNode].iRepr = GIA_VOID;
        pReprs[rootNode].fProved = 0;
    }

    // 检查是否所有 inputNodes 都已处理
    if (inputIndex != numInputs)
    {
        printf("Warning: Not all inputNodes were processed. Processed %d of %d.\n", inputIndex, numInputs);
    }

    return pReprs;
}
void PrintDanglingNodes(Vec_Int_t *vDangles) {
    // 检查返回值是否为空
    if (vDangles == NULL) {
        printf("No dangling nodes found.\n");
        return;
    }
    ...
    // 遍历并打印所有值
    printf("Dangling node IDs: ");
    int entry;
    int i;
    Vec_IntForEachEntry(vDangles, entry, i) {
        printf("%d ", entry);
    }
    printf("\n");
}

static inline void Gia_ObjClearRepr(Gia_Man_t * p, Gia_Obj_t * pNode)
{
    assert(p->pReprs != NULL);
    int id = Gia_ObjId(p, pNode);  // 获取 pNode 的 ID
    p->pReprs[id].iRepr = id;  // 将对应的 iRepr 设置为无效值
}

int Gia_ObjCheckTfi_rec1( Gia_Man_t * p, Gia_Obj_t * pOld, Gia_Obj_t * pNode, Vec_Ptr_t * vVisited )
{
...
//        return 0;
    ...
    // check the children
    if ( Gia_ObjCheckTfi_rec1( p, pOld, Gia_ObjFanin0(pNode), vVisited ) )
        return 1;
    if ( Gia_ObjCheckTfi_rec1( p, pOld, Gia_ObjFanin1(pNode), vVisited ) )
        return 1;
    // check equivalent nodes
    return Gia_ObjCheckTfi_rec1( p, pOld, Gia_ObjNextObj(p, Gia_ObjId(p, pNode)), vVisited );
}
int Gia_ObjCheckTfi1( Gia_Man_t * p, Gia_Obj_t * pOld, Gia_Obj_t * pNode )
{
    Vec_Ptr_t * vVisited;
    Gia_Obj_t * pObj;
    int RetValue, i;
    ...
    RetValue = Gia_ObjCheckTfi_rec1( p, pOld, pNode, vVisited );
...
void Gia_ManMarkValidChoices( Gia_Man_t * p )
{
    Gia_Obj_t * pObj, * pRepr,*pReprNew, * pObjNew;
    int i;

    // 遍历所有对象
    Gia_ManForEachObj( p, pObj, i )
    {
        int id = Gia_ObjId(p, pObj);  // 获取 pObj 的 ID
        pRepr = Gia_ObjReprObj(p, id);  // 获取代表节点

        // 如果 pRepr 不为空且不等于本身，执行逻辑
        if (pRepr != NULL && pRepr != pObj)
        {
            printf("Checking choice %d %d\n", id, Gia_ObjId(p, pRepr));
            // 如果需要，可以添加其他逻辑，比如跳过特定条件的节点
            pReprNew = Gia_ManObj( p, Abc_Lit2Var(pRepr->Value) );
            pObjNew  = Gia_ManObj( p, Abc_Lit2Var(pObj->Value) );

            // if (Gia_ObjCheckTfi1( p, pReprNew, pObjNew ))
            // {
            //     printf("Removing choice %d\n", id);
            //     // Gia_ObjClearRepr(p, pObj);
            //     continue;
            // }

            // 打印测试信息
            printf("Valid choice: Node %d, Repr %d\n", id, Gia_ObjId(p, pRepr));
        }
    }
}


Gia_Man_t * Gia_AigerReadFromMemoryChoice( char * pContents, int nFileSize, int fGiaSimple, int fSkipStrash, int fCheck, Vec_Int_t * aig_orders,Vec_Int_t * len )
{
    Gia_Man_t * pNew, * pTemp;
    Vec_Ptr_t * vNamesIn = NULL, * vNamesOut = NULL, * vNamesRegIn = NULL, * vNamesRegOut = NULL, * vNamesNode = NULL;
    Vec_Int_t * vLits = NULL, * vPoTypes = NULL;
...
//            pCur++;
        while ( pCur < (unsigned char *)pContents + nFileSize )
        {
            ...
            else if (!hasRun)
            {
                ...
                // 获取 dangling nodes
                Vec_Int_t *vDangling =  Gia_ManGetDangling(pNew); // 调用 Gia_Return_dangling 函数
                int numInputs = Vec_IntSize(vDangling);       // 获取 dangling 节点的数量
                printf("Number of dangling nodes: %d\n", numInputs);
                
                //PrintDanglingNodes(vDangling);
                
                long long sum = 0;  // 使用 long long 避免溢出
                int i,entry;
                // void PrintVecInt(Vec_Int_t *len) {
                //     int i, entry;
                
                //     // 打印长度
                //     printf("Vec_Int_t size: %d\n", Vec_IntSize(len));
                
                //     // 遍历 Vec_Int_t 并打印每个元素
                //     printf("Vec_Int_t content: [");
                //     Vec_IntForEachEntry(len, entry, i) {
                //         printf("%d", entry);
                //         if (i < Vec_IntSize(len) - 1) {
                //             printf(", "); // 添加分隔符
                //         }
                //     }
                //     printf("]\n");
                ...
                // 打印最终的叠加结果
                printf("Sum of all dangling node IDs: %lld\n", sum);


                if (numInputs == 0)
                {
                    printf("No dangling nodes found!\n");
                    Vec_IntFree(vDangling); // 释放向量
                    return;
                }
            
                // 动态分配 inputNodes 数组
                int *inputNodes = (int *)malloc(numInputs * sizeof(int));
                for (int i = 0; i < numInputs; i++)
                {
                    inputNodes[i] = Vec_IntEntry(vDangling, i); // 将 dangling 节点存入 inputNodes
                }
                int *rootNodes = (int *)malloc(Vec_IntSize(aig_orders) * sizeof(int));
                for (int i = 0; i < Vec_IntSize(aig_orders); i++)
                {
                    rootNodes[i] = Vec_IntEntry(aig_orders, i);
                }
                int *danglingLenNodes = (int *)malloc(Vec_IntSize(len) * sizeof(int));
                for (int i = 0; i < Vec_IntSize(len); i++)
                {
                    danglingLenNodes[i] = Vec_IntEntry(len, i); // 将 len 的内容逐项复制到 danglingLenNodes
                }
            
                int nSize = Gia_ManObjNum(pNew); // 总节点数
                int  numaignodes= Vec_IntSize(aig_orders);
                int   numDanglingLen =Vec_IntSize(len);
                // 使用 dangling nodes 创建 pReprs
                // (Gia_Man_t *pNew, int *rootNodes, int numRoots, int *inputNodes, int numInputs, int *danglingLenNodes, int numDanglingLen, int nSize)
                pNew->pReprs = CreateReprsWithInputs(pNew,rootNodes,numaignodes, inputNodes, numInputs,danglingLenNodes,  numDanglingLen,nSize);
                // printf("\n=== Printing pReprs (Equivalence Classes) ===\n");
                // Gia_PrintReprs(pNew->pReprs, Gia_ManObjNum(pNew));
                // 生成 pNexts
                // Gia_ManMarkValidChoices(pNew);
                pNew->pNexts = Gia_ManDeriveNexts(pNew);
            
                // 打印 pReprs 和 pNexts
  
                // printf("\n=== Printing pNexts (Equivalence Class Links) ===\n");
                // Gia_PrintNexts(pNew->pNexts, Gia_ManObjNum(pNew));
            
                // 检查是否还有 dangling nodes
                //printf("Has dangling nodes: %d\n", Gia_ManHasDangling(pNew));
            
                // 打印调试信息
                if (fVerbose)
                    printf("Finished reading extension \"e\".\n");
            
                // 释放资源
                Vec_IntFree(vDangling); // 释放 dangling 节点向量
                free(inputNodes);       // 释放动态分配的 inputNodes 数组
                free(rootNodes);
                free(danglingLenNodes);
            }
            // read flop classes
            else if ( *pCur == 'f' )
            {
...
void Gia_PrintReprs(Gia_Rpr_t *pReprs, int nObjs) {
    assert(pReprs != NULL); // Ensure pReprs is not null
    printf("\n=== Printing pReprs (Equivalence Classes) ===\n");
    for (int i = 0; i < nObjs; i++) {
        if (pReprs[i].iRepr == 0 || pReprs[i].iRepr == GIA_VOID) {
            continue; // Skip nodes without valid representatives
        }
        printf("Node %d -> Representative: %d", i, pReprs[i].iRepr);
        if (pReprs[i].fProved) {
            printf(" [Proved]");
        }
        if (pReprs[i].fFailed) {
...
// Function to print pNexts
void Gia_PrintNexts(int *pNexts, int nObjs) {
    assert(pNexts != NULL); // Ensure pNexts is not null
    printf("\n=== Printing pNexts (Equivalence Class Links) ===\n");
    for (int i = 0; i < nObjs; i++) {
        if (pNexts[i] == 0) {
            continue; // Skip nodes without a "next" node
        }
...
void Gia_PrintNextsWithSupport(Gia_Man_t * p, int *pNexts, int nObjs) {
    assert(pNexts != NULL); // 确保 pNexts 非空
    printf("\n=== Printing pNexts (Equivalence Class Links) with Support Nodes ===\n");

    Vec_Int_t *vAndsOrig = Vec_IntAlloc(100);  // 用于存储原节点的 AND 节点
    Vec_Int_t *vCisOrig = Vec_IntAlloc(100);   // 用于存储原节点的 CI 节点
    Vec_Int_t *vAndsChoice = Vec_IntAlloc(100); // 用于存储选择节点的 AND 节点
    Vec_Int_t *vCisChoice = Vec_IntAlloc(100);  // 用于存储选择节点的 CI 节点

    int totalSameAnds = 0;  // 用于计算所有原节点和选择节点之间相同的 AND 节点的总数量
    int totalDiffAnds = 0;  // 用于计算所有原节点和选择节点之间不同的 AND 节点的总数量
    ...
    for (int i = 0; i < nObjs; i++) {
        ...
        // 收集并排序原节点的支持节点
        Vec_IntClear(vAndsOrig); 
        Vec_IntClear(vCisOrig);
        Gia_ManCollectAndsCis(p, &i, 1, vAndsOrig, vCisOrig);
        Vec_IntSort(vAndsOrig, 0); // 对 AND 节点排序
        Vec_IntSort(vCisOrig, 0);  // 对 CI 节点排序

        // 收集并排序选择节点的支持节点
        Vec_IntClear(vAndsChoice); 
        Vec_IntClear(vCisChoice);
        Gia_ManCollectAndsCis(p, &pNexts[i], 1, vAndsChoice, vCisChoice);
        Vec_IntSort(vAndsChoice, 0); // 对 AND 节点排序
        Vec_IntSort(vCisChoice, 0);  // 对 CI 节点排序

        // 打印原节点的 CI 支持节点和其大小
        printf("  CI Nodes for Original Node %d (size %d): ", i, Vec_IntSize(vCisOrig));
        int node, j;
        Vec_IntForEachEntry(vCisOrig, node, j) {
            printf("%d ", node);
        }
        printf("\n");

        // 打印选择节点的 CI 支持节点和其大小
        printf("  CI Nodes for Choice Node %d (size %d): ", pNexts[i], Vec_IntSize(vCisChoice));
        Vec_IntForEachEntry(vCisChoice, node, j) {
            printf("%d ", node);
        }
        printf("\n");

        // 打印原节点的 AND 支持节点和其大小
        printf("  AND Nodes for Original Node %d (size %d): ", i, Vec_IntSize(vAndsOrig));
        Vec_IntForEachEntry(vAndsOrig, node, j) {
            printf("%d ", node);
        }
        printf("\n");

        // 打印选择节点的 AND 支持节点和其大小
        printf("  AND Nodes for Choice Node %d (size %d): ", pNexts[i], Vec_IntSize(vAndsChoice));
        Vec_IntForEachEntry(vAndsChoice, node, j) {
            printf("%d ", node);
        }
        printf("\n");
        ...
        int sameAnds = 0;
        int diffAnds = 0;
        int andNodeOrig, andNodeChoice;
        int k, l;

        // 遍历选择节点的 AND，统计不在原节点中的节点作为不同节点
        Vec_IntForEachEntry(vAndsChoice, andNodeChoice, l) {
            int found = 0;
            Vec_IntForEachEntry(vAndsOrig, andNodeOrig, k) {
                if (andNodeChoice == andNodeOrig) {
                    sameAnds++;
                    found = 1;
                    break;
                }
            ...
            if (!found) {
                diffAnds++; // 选择节点中与原节点不同的 AND 节点
            }
        }

        // 打印相同 AND 节点数量
        printf("  Same AND Nodes between Original Node %d and Choice Node %d: %d\n", i, pNexts[i], sameAnds);

        // 打印不同 AND 节点数量
        printf("  Different AND Nodes between Original Node %d and Choice Node %d: %d\n", i, pNexts[i], diffAnds);

        // 累计到总数量
        totalSameAnds += sameAnds;
        totalDiffAnds += diffAnds;
    }

    // 打印总的相同 AND 节点数量
    printf("\nTotal Same AND Nodes across all Original and Choice Nodes: %d\n", totalSameAnds);

    // 打印总的不同 AND 节点数量
    printf("Total Different AND Nodes across all Original and Choice Nodes: %d\n", totalDiffAnds);

    // 释放内存
    Vec_IntFree(vAndsOrig);
    Vec_IntFree(vCisOrig);
    Vec_IntFree(vAndsChoice);
    Vec_IntFree(vCisChoice);
}


Gia_Man_t * Gia_AigerReadchoice( char * pFileName, int fGiaSimple, int fSkipStrash, int fCheck,Vec_Int_t * aig_orders,Vec_Int_t * len )
{
    FILE * pFile;
    Gia_Man_t * pNew;
    char * pName, * pContents;
    ...
    pNew = Gia_AigerReadFromMemoryChoice( pContents, nFileSize, fGiaSimple, fSkipStrash, fCheck,aig_orders,len);
    extern Gia_Man_t * Gia_ManEquivToChoices( Gia_Man_t * p, int nSnapshots );
    pNew = Gia_ManEquivToChoices(pNew, 1);
    ABC_FREE( pContents );
    if ( pNew )
    {
        ABC_FREE( pNew->pName );
