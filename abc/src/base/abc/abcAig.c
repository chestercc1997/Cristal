...
Vec_Ptr_t* Abc_AigCollectDangles(Abc_Aig_t* pMan) {
    // 创建一个用于存储 dangling nodes 的向量
    Vec_Ptr_t* vDangles = Vec_PtrAlloc(100);
    Abc_Obj_t* pAnd;
    int i;

    // 遍历所有的桶，收集没有 fanout 的 AND 节点
    for (i = 0; i < pMan->nBins; i++) {
        Abc_AigBinForEachEntry(pMan->pBins[i], pAnd) {
            if (Abc_ObjFanoutNum(pAnd) == 0) {
                Vec_PtrPush(vDangles, pAnd);
