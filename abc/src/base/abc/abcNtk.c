...
Abc_Ntk_t * Abc_NtkCreateConeseq( Abc_Ntk_t * pNtk, Abc_Obj_t * pNode, char * pNodeName, int fUseAllCis )
{
    Abc_Ntk_t * pNtkNew; 
    Vec_Ptr_t * vNodes;
    Abc_Obj_t * pObj, * pFanin, * pNodeCoNew;
...
{
    //printf Abc_Obj_t * pNode, char * pNodeName 
    // printf("Abc_NtkCreateMffc: Node ID = %d, Node Name = %s\n", pNode->Id, pNodeName);
    Abc_Ntk_t * pNtkNew; 
    Abc_Obj_t * pObj, * pFanin, * pNodeCoNew;
    ...
    if ( !Abc_NtkCheck( pNtkNew ) )
    {
        ...
        // Abc_NtkDelete( pNtkNew ); // 释放网络
        // return NULL;              // 返回 NULL
    }
    return pNtkNew;
}

Abc_Ntk_t * Abc_NtkCreateMffc_cone( Abc_Ntk_t * pNtk, Abc_Obj_t * pNode, char * pNodeName, Vec_Ptr_t * vCone, Vec_Ptr_t * vSupp )
{
    // printf("Abc_NtkCreateMffc: Node ID = %d, Node Name = %s\n", pNode->Id, pNodeName);
    Abc_Ntk_t * pNtkNew;  
    Abc_Obj_t * pObj, * pFanin, * pNodeCoNew;
    ...
    // Create the PIs (Primary Inputs) from the support set
    Vec_PtrForEachEntry( Abc_Obj_t *, vSupp, pObj, i )
    {
        pObj->pCopy = Abc_NtkCreatePi(pNtkNew);
        Abc_ObjAssignName( pObj->pCopy, Abc_ObjName(pObj), NULL );
    ...
    // Create the PO (Primary Output)
    pNodeCoNew = Abc_NtkCreatePo( pNtkNew );
    Abc_ObjAssignName( pNodeCoNew, pNodeName, NULL );

    // Copy the nodes in the cone
    Vec_PtrForEachEntry( Abc_Obj_t *, vCone, pObj, i )
    {
        // If it is an AIG (And-Inverter Graph), add to the hash table
        if ( Abc_NtkIsStrash(pNtk) )
        {
            pObj->pCopy = Abc_AigAnd( (Abc_Aig_t *)pNtkNew->pManFunc, Abc_ObjChild0Copy(pObj), Abc_ObjChild1Copy(pObj) );
        }
    ...
    // Check the integrity of the new network
    if ( !Abc_NtkCheck( pNtkNew ) )
    {
        fprintf( stdout, "Abc_NtkCreateMffc(): Network check has failed.\n" );
        // If the check fails, cleanup and return NULL
