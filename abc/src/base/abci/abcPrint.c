...
void Abc_NtkPrintMffc( FILE * pFile, Abc_Ntk_t * pNtk )
{
    ...
    Abc_NtkForEachObj( pNtk, pNode, i )
        {
            // 打印节点名称和ID
            fprintf(pFile, "Node Name: %s, Node ID: %d\n", Abc_ObjName(pNode), Abc_ObjId(pNode));
        }
...
/**Function*************************************************************
  ...
  Synopsis    [Writes the MFFCs of the nodes.]
...
// void Abc_NtkWriteMffc( Abc_Ntk_t * pNtk, int minConeSize, int c )
// {
//     Abc_Obj_t * pNode;
//     Abc_Ntk_t * pMffcNtk;
//     char pFileName[1024]; // 缓冲区大小根据实际需求调整
//     int i = 1, count = 1;
//     Vec_Int_t * vNodeIds = NULL; // 用于存储节点ID列表

//     if (c) {
//         Abc_NtkStartReverseLevels(pNtk, 0);  // nMaxLevelIncrease = 0
//         // 获取关键节点列表
//         vNodeIds = Abc_NtkCollectCriticalNodeIds(pNtk);
//         if (!vNodeIds) {
//             printf("Error: Failed to collect critical nodes.\n");
//             return;
//         }
//     }

//     // 检查并创建文件夹 dangle_aig
//     const char * folderName = "./dangle_aig";
//     if (access(folderName, F_OK) == -1) // 文件夹不存在
//     {
//         if (mkdir(folderName, 0755) == -1) // 创建文件夹
//         {
...
//     // 创建或覆盖文件 dangle_aig/map_aig.txt
//     const char * mapFileName = "./dangle_aig/map_aig.txt";
//     FILE * pNodeInfoFile = fopen(mapFileName, "w");
//     if (pNodeInfoFile == NULL) {
//         printf("Error: Unable to create %s file.\n", mapFileName);
//         return;
//     }

//     const char * mffcDir = "./mffc";
//     if (access(mffcDir, F_OK) != -1) // 检查文件夹是否存在
//     {
//         // 如果存在，删除整个目录及其内容
//         char command[1024];
//         snprintf(command, sizeof(command), "rm -rf %s", mffcDir);
//         if (system(command) == -1) // 使用系统命令删除目录
//         {
//             printf("Error: Unable to delete existing folder %s.\n", mffcDir);
//             return;
//         }
//     }

//     // 创建新的 mffc 目录
//     if (mkdir(mffcDir, 0755) == -1) // 创建文件夹
//     {
//         printf("Error: Unable to create folder %s.\n", mffcDir);
//         return;
//     }
...
//         // 遍历关键节点
//         int NodeId;
//         Vec_IntForEachEntry(vNodeIds, NodeId, i) {
//             pNode = Abc_NtkObj(pNtk, NodeId);
//             if (!pNode || !Abc_ObjIsNode(pNode)) {
//                 printf("Warning: Invalid node ID %d\n", NodeId);
//                 continue;
//             }
...
//             // 检查是否满足规则：支持集和锥体都大于等于指定值
//             if (Vec_PtrSize(vSupp) >= 2 && Vec_PtrSize(vCone) >= minConeSize) {
//                 if (Abc_ObjIsNode(pNode)) {
//                     int nodeId = Abc_ObjId(pNode) - Abc_NtkPoNum(pNtk); // 计算节点 ID
//                     fprintf(pNodeInfoFile, "%d\n", nodeId); // 只写入 nodeId，每行一个

//                     // 创建节点名称的静态拷贝（1024字节）
//                     char staticNodeName[1024];
//                     strncpy(staticNodeName, Abc_ObjName(pNode), 1023);
//                     staticNodeName[1023] = '\0';

//                     // 生成文件名：mffc文件夹/节点名称.aig
//                     snprintf(pFileName, sizeof(pFileName), "%s/%d.aig", mffcDir, count);

//                     // 创建 MFFC 网络
//                     pMffcNtk = Abc_NtkCreateMffc(pNtk, pNode, staticNodeName);

//                     // 写出 AIG 文件
//                     Io_WriteAiger(pMffcNtk, pFileName, 1, 0, 0);

//                     Abc_NtkDelete(pMffcNtk);
//                 }
//                 count += 1;
//             }
...
//     // 关闭节点信息文件
//     fclose(pNodeInfoFile);
// }
void Abc_NtkWriteMffc( Abc_Ntk_t * pNtk, int minConeSize, int c )
{
    int k = 10; // <<< 可调控的参数，保留 k 个满足条件的节点
    int f = 1; // <<< 开关：1 表示保留最后 k 个；0 表示保留最前 k 个

    Abc_Obj_t * pNode;
    Abc_Ntk_t * pMffcNtk;
    char pFileName[1024];
    int i = 1, count = 0;
    Vec_Int_t * vNodeIds = NULL;
    Vec_Int_t * vKeepNodeIds = Vec_IntAlloc(100);
    ...
    const char * folderName = "./dangle_aig";
    if (access(folderName, F_OK) == -1 && mkdir(folderName, 0755) == -1) {
        printf("Error: Unable to create folder %s.\n", folderName);
        return;
    }
    ...
    if (c) {
        ...
        Vec_IntForEachEntry(vNodeIds, NodeId, i) {
            ...
            Vec_Ptr_t * vSupp = Vec_PtrAlloc(100);
            Abc_NodeDeref_rec(pNode);
            Abc_NodeMffcConeSupp(pNode, vCone, vSupp);
            Abc_NodeRef_rec(pNode);

            if (Vec_PtrSize(vSupp) >= 2 && Vec_PtrSize(vCone) >= minConeSize) {
                Vec_IntPush(vKeepNodeIds, Abc_ObjId(pNode));
                nodeCount++;
            }
    ...
    int startIdx = 0;
    int endIdx = nodeCount;

    if (k < nodeCount) {
        if (f)
            startIdx = nodeCount - k;  // 最后 k 个
        else
            endIdx = k;                // 最前 k 个
    }

    for (i = startIdx; i < endIdx; i++) {
        int NodeId = Vec_IntEntry(vKeepNodeIds, i);
        pNode = Abc_NtkObj(pNtk, NodeId);
        if (!pNode || !Abc_ObjIsNode(pNode))
            continue;

        int nodeId = Abc_ObjId(pNode) - Abc_NtkPoNum(pNtk);
        fprintf(pNodeInfoFile, "%d\n", nodeId);
        ...
        snprintf(pFileName, sizeof(pFileName), "%s/%d.aig", mffcDir, offset);
    ...
    fclose(pNodeInfoFile);
    Vec_IntFree(vKeepNodeIds);
    if (vNodeIds) Vec_IntFree(vNodeIds);
}


void Abc_NtkWriteMffc_a(Abc_Ntk_t * pNtk, int minConeSize, int k, int maxConeSize,int maxFindCount)
{ 
Abc_Obj_t * pNode;
    Abc_Ntk_t * pMffcNtk;
    char pFileName[1024]; // 用于生成文件名
    int i = 1, count = 1; // 节点计数器
    int j, id;
    Vec_Int_t * vNodeIds = NULL; // 用于存储节点 ID 列表
    ...
    // 检查并创建 mffc 文件夹
    const char * mffcDir = "./mffc";
    if (access(mffcDir, F_OK) != -1) // 如果文件夹存在
    {
        char command[1024];
        snprintf(command, sizeof(command), "rm -rf %s", mffcDir);
        if (system(command) == -1) // 删除文件夹
        {
            printf("Error: Unable to delete existing folder %s.\n", mffcDir);
            fclose(pNodeInfoFile);
            return;
    ...
    if (mkdir(mffcDir, 0755) == -1) // 创建 mffc 文件夹
    {
        printf("Error: Unable to create folder %s.\n", mffcDir);
        fclose(pNodeInfoFile);
        return;
    ...
    // if (k != 1)
    // {
        Vec_Int_t * vConeId, * vSuppId;
        Vec_Ptr_t * vCone, * vSupp;

        // 复制网络
        Abc_Ntk_t * pNtkCopy = Abc_NtkDup(pNtk);
        int count_node =0;
        Abc_NtkForEachNode(pNtk, pNode, i)
        {
            if (maxFindCount!=-1 && count>maxFindCount) break;
            // 在复制网络中找到与 pNode 拥有相同 ID 的节点
            count_node ++;
            Abc_Obj_t * pNodeCopy = Abc_NtkObj(pNtkCopy, Abc_ObjId(pNode));
            if (pNodeCopy == NULL)
            {
                printf("Error: Could not find node with ID %d in pNtkCopy.\n", Abc_ObjId(pNode));
                continue;
            }

            // 分配存储锥体和支持集的向量
            vCone = Vec_PtrAlloc(100);    // 存储锥体对象
            vSupp = Vec_PtrAlloc(100);    // 存储支持集对象
            vConeId = Vec_IntAlloc(100); // 存储锥体对象的 ID
            vSuppId = Vec_IntAlloc(100); // 存储支持集对象的 ID

            // 递归计算锥体和支持集
            Abc_NodeDerefKToZero_rec(pNodeCopy, k);
            Abc_NodeMffcConeSuppId(pNodeCopy, vConeId, vSuppId);
            pNtkCopy = Abc_NtkDup(pNtk);
            // 根据 ID 在原网络中找到对应的对象
            Vec_IntForEachEntry(vConeId, id, j)
            {
                Abc_Obj_t * pObj = Abc_NtkObj(pNtk, id);
                if (pObj) Vec_PtrPush(vCone, pObj);
            }

            Vec_IntForEachEntry(vSuppId, id, j)
            {
                Abc_Obj_t * pObj = Abc_NtkObj(pNtk, id);
                if (pObj) Vec_PtrPush(vSupp, pObj);
            }
        if (maxConeSize==-1){
            // 检查锥体和支持集的大小
            if (Vec_PtrSize(vSupp) >= 2 && Vec_PtrSize(vCone) >= minConeSize)
            {
                if (Abc_ObjIsNode(pNode))
                {
                    ...
                    // 生成文件名并写出 AIG 文件
                    snprintf(pFileName, sizeof(pFileName), "%s/%d.aig", mffcDir, count);
                    Abc_Ntk_t * pMffcNtk = Abc_NtkCreateMffc_cone(pNtk, pNode, staticNodeName, vCone, vSupp);
                    Io_WriteAiger(pMffcNtk, pFileName, 1, 0, 0);
                    Abc_NtkDelete(pMffcNtk);
        ...
        else{            if (Vec_PtrSize(vSupp) >= 2 && Vec_PtrSize(vCone) >= minConeSize && Vec_PtrSize(vCone)<=maxConeSize)
            {
                if (Abc_ObjIsNode(pNode))
                {
                    // 计算节点 ID 并写入文件
...
void Abc_NtkWritecpconeWithLimits(Abc_Ntk_t * pNtk, int nLevels, int maxConeSizeLimit, int minConeSizeLimit, int K)
{
    /*---------- 替换关键节点为采样节点 ----------*/
    Vec_Int_t * vSampledNodeIds = Abc_NtkCollectSampledNodeIds(pNtk, K);
    if (!vSampledNodeIds) {
        printf("Error: Failed to collect sampled nodes.\n");
        return;
    }
    ...
    Abc_Ntk_t * pMffcNtk;
    char pFileName[1024];
    int i, NodeId, count = 1;
    //print maxConeSizeLimit and min 
    printf("maxConeSizeLimit = %d, minConeSizeLimit = %d\n", maxConeSizeLimit, minConeSizeLimit);
    /*---------- 目录和文件初始化 ----------*/
    // 创建dangle_aig目录
    const char * folderName = "./dangle_aig";
    if (access(folderName, F_OK) == -1) // 文件夹不存在
    ...
    if (mkdir(mffcDir, 0755) == -1) {
        printf("Error: Unable to create folder %s.\n", mffcDir);
        fclose(pNodeInfoFile);
        Vec_IntFreeP(&vSampledNodeIds);
        return;
    }

    /*---------- 关键节点处理 ----------*/
    Vec_IntForEachEntry(vSampledNodeIds, NodeId, i)
    {
        // 转换Literal为节点指针
        pNode = Abc_NtkObj(pNtk, NodeId);
        if (!pNode || !Abc_ObjIsNode(pNode)) {
            printf("Warning: Invalid critical node ID %d\n", NodeId);
            continue;
        }
        ...
        /*----- 创建锥体网络 -----*/
        pMffcNtk = Abc_NtkCreateCone(pNtk, pNode, Abc_ObjName(pNode), 1);
        if (pMffcNtk) {
            // 应用Topmost优化
            Abc_Ntk_t * pTopmost = Abc_NtkTopmost(pMffcNtk, nLevels);
            if (pTopmost) {
                // 计算pTopmost的节点数量
                int coneSize = Abc_NtkObjNum(pTopmost);
    
                // 根据锥体大小限制过滤节点
                if (coneSize < minConeSizeLimit || coneSize > maxConeSizeLimit) {
                    // 不满足大小限制，释放pTopmost并跳过
                    Abc_NtkDelete(pTopmost);
                    Abc_NtkDelete(pMffcNtk);
                    continue;
                }
                snprintf(pFileName, sizeof(pFileName), "%s/%d.aig", mffcDir, count);
                ...
                // 写入节点ID到map文件
                fprintf(pNodeInfoFile, "%d\n", Abc_ObjId(pNode) - Abc_NtkPoNum(pNtk));
    
                // 写出AIG文件
                Io_WriteAiger(pTopmost, pFileName, 1, 0, 0);
                Abc_NtkDelete(pTopmost);
            }
            Abc_NtkDelete(pMffcNtk);
        }
