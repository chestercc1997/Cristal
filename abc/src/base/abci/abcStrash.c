...
int Abc_NtkAppend1( Abc_Ntk_t * pNtk1, Abc_Ntk_t * pNtk2, int fAddPos )
{
    Abc_Obj_t * pObj;
    char * pName;
    int i, nNewCis;
    ...
    {   
        void* pTemp;
        pTemp = Abc_NtkFindCi(pNtk1, Abc_ObjName(pObj));
        if (pTemp != NULL)
        {
            pObj->pCopy = pTemp;
        }
        ...
        {
            // 如果找不到 CI，再尝试查找 Node
            pTemp = Abc_NtkFindNode(pNtk1, Abc_ObjName(pObj));
            if (pTemp != NULL)
            {
                pObj->pCopy = pTemp;
            }
        }
        //int iNodeId =Nm_ManFindIdByNameTwoTypes( pNtk1->pManName, Abc_ObjName(pObj), ABC_OBJ_PI, ABC_OBJ_NODE);
        // pObjOld = Abc_NtkObj( pNtk1, iNodeId );
        // pObj->pCopy=Abc_AigAnd( (Abc_Aig_t *)pNtk1->pManFunc, Abc_ObjChild0Copy(pObj), Abc_ObjChild1Copy(pObj) );
        // Nm_ManFindIdByName(pNtk1->pManName, Abc_ObjName(pObj), ABC_OBJ_NODE);
...
Abc_Obj_t * Abc_NtkTopmost_rec( Abc_Ntk_t * pNtkNew, Abc_Obj_t * pNode, int LevelCut )
{
    ...
    // 如果节点的 Level 小于等于 LevelCut，创建 PI 节点
    if ( pNode->Level <= (unsigned)LevelCut )
    {
        // 创建 PI 节点并复制名称
        pNode->pCopy = Abc_NtkCreatePi( pNtkNew );
        Abc_ObjAssignName( pNode->pCopy, Abc_ObjName(pNode), NULL );  // 复制名称
        return pNode->pCopy;
    }
...
Abc_Ntk_t * Abc_NtkTopmost( Abc_Ntk_t * pNtk, int nLevels )
{
    ...
    Abc_NtkForEachCo( pNtk, pObj, i )
    {
        ...
        // 如果返回 NULL（遇到 CO），删除新网络并返回 NULL
        if ( pObjNew == NULL )
        {
            printf("Abc_NtkTopmost: CO node found below the level cut. Returning NULL.\n");
...
Abc_Obj_t * Abc_NtkSuppCut_rec( Abc_Ntk_t * pNtkNew, Abc_Obj_t * pNode, int nSuppMax )
{
    Abc_Obj_t * pFanin;
    int SuppSize,i;
    ...
    // 计算当前节点支持集大小
    SuppSize = Abc_NodeSuppSize(pNode);

    // 支持集超过阈值则转换为PI
    if ( SuppSize > nSuppMax )
    {
        Abc_Obj_t * pPiNew = Abc_NtkCreatePi(pNtkNew);
        Abc_ObjAssignName(pPiNew, Abc_ObjName(pNode), NULL);
        return pNode->pCopy = pPiNew;
    }

    // 递归处理扇入
    Abc_ObjForEachFanin( pNode, pFanin, i )
        Abc_NtkSuppCut_rec( pNtkNew, pFanin, nSuppMax );

    // 构建当前节点
    return pNode->pCopy = Abc_AigAnd( (Abc_Aig_t *)pNtkNew->pManFunc, 
        Abc_ObjChild0Copy(pNode), Abc_ObjChild1Copy(pNode) );
}

/**Function*************************************************************
  Synopsis    [支持集驱动的网络剪裁入口函数]
  Description [根据支持阈值重构网络拓扑]
  SideEffects []
  SeeAlso     []
***********************************************************************/
Abc_Ntk_t * Abc_NtkSuppCut( Abc_Ntk_t * pNtk, int nSuppMax )
{
    Abc_Ntk_t * pNtkNew;
    ...
    Abc_NtkForEachCo( pNtk, pObj, i )
    {
        ...
        pDriver = Abc_NtkSuppCut_rec( pNtkNew, pDriver, nSuppMax );
        pDriver = Abc_ObjNotCond( pDriver, Abc_ObjFaninC0(pObj) );
        Abc_ObjAddFanin( Abc_NtkCreatePo(pNtkNew), pDriver );
    }

    // 校验网络完整性
    if ( !Abc_NtkCheck( pNtkNew ) )
    {
        printf( "Abc_NtkSuppCut: Network verification failed.\n" );
        Abc_NtkDelete( pNtkNew );
        return NULL;
    }
    return pNtkNew;
...
/**Function*************************************************************
  Synopsis    [计算节点支持集大小]
  Description [快速计算驱动当前节点的PI数量]
  SideEffects [修改遍历标记]
  SeeAlso     []
***********************************************************************/
int Abc_NodeSuppSize( Abc_Obj_t * pNode )
{
    ...
    // 初始化遍历系统
    Abc_NtkIncrementTravId( pNtk );
    Abc_NodeSetTravIdCurrent( pNtk);

    // 执行支持集计算
    Abc_NodeSupportSize_rec( pNode, &Counter );
    return Counter;
}
...
/**Function*************************************************************
  ...
  SeeAlso     []
***********************************************************************/
void Abc_NodeSupportSize_rec( Abc_Obj_t * pNode, int * pCounter )
{
    int i;
    Abc_Obj_t * pFanin;  // 初始化变量
    // 常量或已访问节点直接返回
    if ( Abc_NodeIsTravIdCurrent(pNode) || Abc_AigNodeIsConst(pNode) )
        return;
    ...
    // 递归处理扇入
    Abc_ObjForEachFanin( pNode, pFanin, i )
        Abc_NodeSupportSize_rec( pFanin, pCounter );
}
/**Function*************************************************************
