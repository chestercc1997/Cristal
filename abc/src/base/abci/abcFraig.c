...
int Abc_NtkAigStore( Abc_Ntk_t * pNtkAdd )
{
    Vec_Ptr_t * vStore;
    Abc_Ntk_t * pNtk;
    // create the network to be stored
...
void Abc_NtkAigStoreClean()
{
    ...
    if (vStore == NULL)
    {
        printf("Warning: vStore is NULL. Nothing to clean.\n");
        return; // 如果 vStore 是 NULL，直接退出函数
    }

    // 遍历 vStore，删除除第一个以外的所有网络
    Vec_PtrForEachEntry( Abc_Ntk_t *, vStore, pNtk, i )
    {
        if (i == 0) // 保留第一个网络
            continue;

        if (pNtk == NULL) // 处理可能的 NULL 条目
        {
            printf("Warning: Encountered NULL network in vStore at index %d.\n", i);
            continue;
        }
    ...
    // 保留第一个网络，将其重新存储到 vStore
    if (Vec_PtrSize(vStore) > 0)
    {
        pNtk = Vec_PtrEntry(vStore, 0); // 获取第一个网络
        Vec_PtrClear(vStore);          // 清空 vStore
        Vec_PtrPush(vStore, pNtk);     // 重新将第一个网络添加回 vStore
    }
    else
    {
        // 如果 vStore 没有网络，打印警告
        printf("Warning: vStore is empty. Nothing to clean.\n");
    }
}
/**Function*************************************************************
