...
//static Vec_Int_t * Abc_NtkCollectCriticalNodeIds( Abc_Ntk_t * pNtk );
...
Vec_Int_t * Abc_NtkCollectCriticalNodeIds( Abc_Ntk_t * pNtk )
{
    Abc_Obj_t * pNode;
    int i, co = Abc_NtkCoNum(pNtk);  // 提前计算常量值
    Vec_Int_t * vCriticalNodeIds = Vec_IntAlloc(100);
    ...
    Abc_NtkForEachNode(pNtk, pNode, i)
    {
        ...
        // 直接存储原始节点ID
        Vec_IntPush(vCriticalNodeIds, Abc_ObjId(pNode));
    }

    /*---------- 文件输出处理 ----------*/
    const char *folderName = "./output_cp_file";
    const char *fileName = "./output_cp_file/cp_lit.txt";
    FILE *outputFile = NULL;

    // 创建目录（如果不存在）
    if (access(folderName, F_OK) == -1 && mkdir(folderName, 0755) == -1) {
        fprintf(stderr, "Error: Cannot create directory %s\n", folderName);
        Vec_IntFree(vCriticalNodeIds);
        return NULL;
    }
    ...
    {
        int NodeId, lit;
        Vec_IntForEachEntry(vCriticalNodeIds, NodeId, i)
        {
            // 转换为关键节点指针
            Abc_Obj_t * pCriticalNode = Abc_NtkObj(pNtk, NodeId - co);
            if (!pCriticalNode) {
                fprintf(stderr, "Warning: Invalid node conversion for ID %d\n", NodeId);
                continue;
            }
            
            // 保持原有Literal写入逻辑
            lit = Abc_ObjToLit(pCriticalNode);
    ...
    printf("Collected %d critical nodes\n", Vec_IntSize(vCriticalNodeIds));
    return vCriticalNodeIds;
}


Vec_Int_t * Abc_NtkCollectSampledNodeIds(Abc_Ntk_t * pNtk,int K)
{
    Abc_Obj_t * pNode;
    int i, count = 0;
    Vec_Int_t * vSampledNodes = Vec_IntAlloc(100);
    ...
    Abc_NtkForEachNode(pNtk, pNode, i)
    {
        ...
        // 存储原始节点ID
        Vec_IntPush(vSampledNodes, Abc_ObjId(pNode));
    }
    ...
    {
        int NodeId, lit;
        Vec_IntForEachEntry(vSampledNodes, NodeId, i)
        {
            Abc_Obj_t * pSampledNode = Abc_NtkObj(pNtk, NodeId);
            if (!pSampledNode) {
                fprintf(stderr, "Warning: Invalid node ID %d\n", NodeId);
                continue;
            }
            lit = Abc_ObjToLit(pSampledNode);
            fprintf(outputFile, "%d\n", lit);
        }
        fclose(outputFile);
    }
    ...
    printf("Sampled %d nodes (every %dth node)\n", Vec_IntSize(vSampledNodes), K);
    return vSampledNodes;
}
////////////////////////////////////////////////////////////////////////
///                       END OF FILE                                ///
////////////////////////////////////////////////////////////////////////
