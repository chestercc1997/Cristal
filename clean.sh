...
# Special case for e-rewriter: remove .json if it exists
...
if [ -d "abc_old" ]; then
  rm -f abc_old/*.eqn 2>/dev/null
  rm -f abc_old/stats.txt 2>/dev/null
  echo -e "${GREEN}Cleaned abc_old directory${RESET}"
fi
