...
***********************************************************************/
void Aig_ObjClearEquiv( Aig_Man_t * p, Aig_Obj_t * pNode )
{
    assert( p->pEquivs != NULL );

    Aig_Obj_t *pCurrent, *pNext;

    // 遍历链表，清空所有等效节点的关系
    pCurrent = p->pEquivs[pNode->Id];
    while (pCurrent != NULL)
    {
        pNext = p->pEquivs[pCurrent->Id]; // 记录下一个节点
        p->pEquivs[pCurrent->Id] = NULL; // 清除当前节点的等效关系
        pCurrent = pNext;                // 移动到下一个节点
    }

    // 最终保留 pNode 的自身关系
    p->pEquivs[pNode->Id] = NULL;
}
...
static inline Aig_Obj_t * Aig_ObjGetLastEquiv(Aig_Man_t * pMan, Aig_Obj_t * pObj)
{
    Aig_Obj_t *pLast = pObj;
    Aig_Obj_t *pTemp = pObj;
    
    // 遍历到等价链表的末尾
    while ((pTemp = Aig_ObjEquiv(pMan, pTemp))) {
        pLast = pTemp;
    }
    
    // 验证最后一个节点的ID必须小于原始节点ID
    // assert(pLast->Id < pObj->Id); // 断言失败说明存在非法等价关系
    
    return pLast;
}
void PrintEquivList(Aig_Man_t * pMan)
{
    Aig_Obj_t *pObj;
    int i;
    Aig_ManForEachNode(pMan, pObj, i) {
        Aig_Obj_t *pLast = Aig_ObjGetLastEquiv(pMan, pObj);
        printf("Node %d: LastEquiv=%d -> ", pObj->Id, pLast->Id);
...
{
    Abc_Ntk_t * pNtkNew;
    Aig_Obj_t * pObj, * pTemp, *pRoot, * pRepr;
    int i;
    ...
    //     // 遍历当前节点的等价链表
    //     if ((pRepr = Aig_ObjGetLastEquiv(pMan, pObj))) {
    //         Aig_ObjClearEquiv( pMan, pObj );
    //         printf("remove equiv %d %d\n", pObj->Id, pRepr->Id);
    //     }
    // }
